// OpenMP parallelized program for trapezoidal rule using reduction clause
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <math.h>
...
int main(int argc, char *argv[])	{
	...
	n = 72;	// number of divisions for integration, should be evenly divisible with the number of threads used
	a = 0.0;	// integration lower limit
	b = PI;	// integration upper limit
	integration_result = 0.0;	
	
	#pragma omp parallel num_threads(num_thread)	reduction(+:integration_result)
	{	
		integration_result = integration_result + trap_rule(a, b, n);
	}
